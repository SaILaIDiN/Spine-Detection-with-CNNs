...
def load_json_log_filter_top_N(json_log, mode=None, top_N=1, lr=None, momentum=None, weight_decay=None, dropout=None):
...
def prep_Def_DETR_with_SGD_logs_top_N(test_content, list_train_csv, list_special_term, list_da_modes,
                                      list_model_type, list_learning_rate, list_weight_decay,
                            ...
                            for momentum in list_momentum:
                                for weight_decay in list_weight_decay:
                                    run_name = 'lr_' + str(lr) + '_warmup_' + str(warm_up) + \
                                               '_dropout_' + str(dropout) + '_momentum_' + str(momentum) +\
                                               '_L2_' + str(weight_decay) + str(special_term)
                                    test_path = \
                                        f"{model_type}_Plot_Analysis/{test_content.split('/')[0]}/{da_mode}/{run_name}"
                                    log_path = test_path + "/None.log.json"
...
def scatter_plot_loss_by_single_param(df, key_param, xlabel, ylabel, title=None, output=None, custom_dict=None):
    ...
    # ax.set_ylim(bottom=0.3, ymax=0.6)  # for Cascade-RCNN
    # ax.set_ylim(bottom=1.62, ymax=1.675)  # for VFNet
    ax.set_ylim(bottom=1.55, ymax=2.7)  # for VFNet
    # ax.set_ylim(bottom=6.5, ymax=7.5)  # for Def-DETR
    # ax.set_ylim(bottom=6.5, ymax=15)  # for Def-DETR
...
if __name__ == "__main__":
    ...
    # list_weight_decay = ['0.3', '0.03', '0.0003', '3e-06']
    # list_warm_up = [None]
    ...
    df_total_top_1 = prep_Cascade_or_VFNet_with_SGD_logs_top_N(
        ...
        list_weight_decay, list_warm_up, list_momentum, top_N=1)
    ...
    # list_dropout = ['0.1']
    #
    # df_total_top_1 = prep_Def_DETR_with_SGD_logs_top_N(
    #     test_content, list_train_csv, list_special_term, list_da_modes, list_model_type, list_learning_rate,
    #     list_weight_decay, list_warm_up, list_momentum, list_dropout, top_N=4)
    # print(df_total_top_1.to_string())
    ...
    scatter_plot_loss_by_single_param(df_total_top_1, "weight_decay", xlabel="weight decay", ylabel="val loss",
                                      ...
                                      custom_dict={'0.3': 0, '0.03': 1, '0.0003': 2, '3e-06': 3})
